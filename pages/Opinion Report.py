...
from metrics import consistency_of_opinion, positive_opinion, negative_opinion


st.title("Consistency Of Opinion Breakdown Report")
...
if "chat_data" in st.session_state:
    ...
    # Pie Chart
    if result["Total Opinions"] == 0:
        st.write("### No Significant Opinions Detected")
        st.write(result["Consistency"])
    else:
        st.write("### Distribution of Positive and Negative Opinions")
        
        fig = go.Figure(
            data=[
                go.Pie(
                    labels=["Positive Opinions", "Negative Opinions"],
                    values=[result["Positive Opinions"], result["Negative Opinions"]],
        ...
        fig.update_layout(
            title="Opinion Word Distribution",
            annotations=[
                dict(
        ...
        st.plotly_chart(fig)
        st.write(f"##### Consistency of Opinion Tone: {result['Consistency']}")
